...
Configured for Hagrid - takes in parameters tmax, rw, dt2 from command line
...
tmax_in = float(sys.argv[1])
rw_in = float(sys.argv[2])
dt2_in = float(sys.argv[3])
...
# EWS parameters
dt2 = dt2_in # spacing between time-series for EWS computation
rw = rw_in # rolling window
...
df_ews.loc[1:5].to_csv('ews_singles.csv')

# Export power spectra of first 5 realisations
df_pspec.loc[1:5].to_csv('pspec.csv')

# Export aggregates
df_ews_means.to_csv('ews_means.csv')
df_ews_deviations.to_csv('ews_deviations.csv')

# Export Kendall tau values
df_ktau.to_csv('ktau.csv')
