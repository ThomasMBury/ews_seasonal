...
Simulate transient simulations of the Ricker model (with dem and env noise)
as the density dependence (alpha) increase
...
# a = 0.01   	# Strength of density dependent effects
r = 1.2         # Growth rate
...
def de_fun(state, control, noise):
    ...
    xnew = x * np.exp(r - a * x) + eps_dem*np.sqrt(x) + eps_env*x
...
b = pd.Series(np.linspace(al, ah, len(t)),index=t)
# Time at which bifurcation occurs
tcrit = b[b > acrit].index[1]
